...
def get_current_week_num():  # номер текущей недели
    ...
    result = soup.find("div", class_="mainpage").find("div").find("div").find("div",
                                                                              class_="bonus_cart-title")
    # строка с номером недели
    if "экзаменационная" in result.get_text():
        return 0  # экзаменационная неделя
    try:
        week_num = int(re.findall(pattern="[0-9]+", string=result.get_text().split(",")[1])[
                           0])  # поиск числа (номера недели) (работает только во время учебы)
        return week_num
    except IndexError:
        return -1  # каникулы
...
def group_exists(group_name):  # проверка, существует ли группа
    if get_schedule(group_name) is None:
        return False
    else:
        return True
...
def download_schedule(course):  # скачивание расписания по номеру курса
    ...
    pattern = r"unknown"
    if course == 1:
        pattern = r"/ИИТ_1.+.xlsx"
    elif course == 2:
        pattern = r"/ИИТ_2.+.xlsx"
    elif course == 3:
        pattern = r"/ИИТ_3.+.xlsx"
...
def get_schedule_day(group_name, week_num, week_day_num):  # обработка расписания на день
    ...
    for lesson in day:
        ...
        for i in range(len(subject)):
            ...
            if substr_subject_num[0].isdigit() and re.fullmatch(pattern=DASH_INCLUDE,
                                                                string=substr_subject_num):  # номера недель, на которых есть занятия через тире
                ...
                if week_num_from > week_num or week_num_to < week_num:
                    substr_subject = "—"
                    empty_lesson()
            elif substr_subject_num[0].isdigit() and re.fullmatch(pattern=COMMA_INCLUDE,
                                                                  string=substr_subject_num):  # номера недель, на которых есть занятия через запятую
                ...
                if week_num != week_num_include:
                    substr_subject = "—"
                    empty_lesson()
            elif substr_subject_num[0:2] == "кр" and re.fullmatch(pattern=DASH_EXCLUDE,
                                                                  string=substr_subject_num):  # номера недель, на которых нет занятий через тире
                week_num_from = int(substr_subject_num.split(" ")[1].split("-")[0])
                week_num_to = int(substr_subject_num.split(" ")[1].split("-")[1])
                if week_num_from <= week_num <= week_num_to:
                    substr_subject = "—"
                    empty_lesson()
            elif substr_subject_num[0:2] == "кр" and re.fullmatch(pattern=COMMA_EXCLUDE,
                                                                  string=substr_subject_num):  # номера недель, на которых нет занятий через запятую
