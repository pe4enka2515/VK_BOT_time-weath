...
def text_to_png(array):  # перевод текста на картинку
    ...
    main_font = ImageFont.truetype("fonts/Arial.ttf", 15)
...
def get_weather_current():  # получить текущую погоду
    ...
    if os.path.isfile(filepath):
        with open(filepath, "r") as f:
            info = json.load(f)
            if (datetime.datetime.now() - datetime.datetime.fromtimestamp(int(info["dt"])) < datetime.timedelta(0, 0, 0,
                                                                                                                0,
                                                                                                                30)):
                # погода обновлена в последние полчаса
    ...
    if os.path.isfile(filepath):
        with open(filepath, "r") as f:
            ...
            if datetime.datetime.now() - datetime.datetime.fromtimestamp(
                    int(info["list"][0]["dt"])) < datetime.timedelta(0, 0, 0, 0, 0,
                                                                     3):  # погода обновлена в последние 3 часа
...
def create_weather_5days_png():  # перевод информации о погоде за 5 дней из текста на картинку
    if get_weather_5days()[1] or True:  # убрать true
        ...
        if datetime.datetime.fromtimestamp(info[len(info) - 1]['dt']).time() <= datetime.time(
                hour=12):  # время до 12, значит прогноз начинается с текущего дня
            dt_1day = datetime.datetime.fromtimestamp(info[0]['dt'])  # дата первого дня в (dt)
        else:
            dt_1day = datetime.datetime.fromtimestamp(info[0]['dt']) + datetime.timedelta(
                days=1)  # дата первого дня в (dt)
