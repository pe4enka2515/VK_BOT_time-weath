...
keyboard_schedule.add_button(MESSAGE_SCHEDULE_WEEK_CURRENT, color=VkKeyboardColor.PRIMARY)
...
def main():
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.text and event.to_me:
            ...
            if user in show_weather:
                show_weather[user] = False
            if user not in show_schedule:
            ...
            if text in ["начать", "привет", "бот"]:  # приветствие
                ...
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(),
                                 message=MESSAGE_GREETING_TEXT)
            elif (text in ["расписание занятий",
                           "расписание"] or text in DAYS_WEEK) and user not in group_dict:  # вывод ввода группы
                ...
                show_schedule[user] = True
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_schedule.get_keyboard(),
                                 message=MESSAGE_GROUP_SHOW_TEXT + group)
            elif re.fullmatch(GROUP_NAME_PATTERN, text.upper()):  # ввод группы
                if not group_exists(text.upper()):
                    vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(),
                                     message=MESSAGE_GROUP_NAME_WRONG_TEXT.format(text.upper()))
                    continue
                show_schedule[user] = True
                group = text.upper()
                ...
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_schedule.get_keyboard(),
                                 message=MESSAGE_GROUP_SHOW_TEXT + group)
                schedule_day = create_schedule_day_heading(group, current_week - 1, day_week_num,
                ...
                message = array_to_string(schedule_day)
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(), message=message)
            elif re.fullmatch(GROUP_NAME_DAY_WEEK_PATTERN,
                              text.upper()):  # расписание на день недели (с указанием группы)
                group_temp = text.upper().split(" ")[1]
                day_week = text.split(" ")[0]
                day_week_num = DAYS_WEEK[day_week]
                if day_week_num == 6:
                    message = MESSAGE_SCHEDULE_SUNDAY
                    vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(), message=message)
                    continue
                if not group_exists(group_temp):
                    vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(),
                                     message=MESSAGE_GROUP_NAME_WRONG_TEXT.format(group_temp))
                    continue
                if current_week == 0:
                    message = MESSAGE_SCHEDULE_SESSION
                    vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(), message=message)
                ...
                if current_week == -1:
                    ...
                    continue
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_schedule.get_keyboard(),
                                 message=MESSAGE_GROUP_SHOW_TEXT + group_temp)
                schedule_day = create_schedule_day_heading(group_temp, current_week - 1, day_week_num,
                                                           text.split(" ")[0])  # предыдущая неделя
                message = array_to_string(schedule_day)
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(), message=message)
                schedule_day = create_schedule_day_heading(group_temp, current_week, day_week_num,
                                                           text.split(" ")[0])  # текущая неделя
                message = array_to_string(schedule_day)
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(), message=message)
                ...
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(),
                                 message=MESSAGE_GROUP_SHOW_TEXT + group)
            elif text in ["погода", "погода в москве"] and not show_weather.get(user):  # вывод выбора периода погоды
                show_weather[user] = True
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_weather.get_keyboard(),
                                 message=MESSAGE_WEATHER)
                ...
                show_weather[user] = False
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(),
                                 message=MESSAGE_UNAVALAIBLE_COMMAND)
                ...
                vk.messages.send(user_id=user, random_id=0, keyboard=keyboard_menu.get_keyboard(),
                                 message=MESSAGE_UNKNOWN_COMMAND)
